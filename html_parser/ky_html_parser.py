        ...
        for header_tag in self.soup.body.find_all():
            if re.search('constitution', self.html_file_name):
                ...
                if header_tag.get("class") == [self.class_regex["title"]]:
                    ...
                    if re.search("^(CHAPTER)|^(Chapter)", header_tag.text):
                    ...
                    elif re.search("^(Article|SUBCHAPTER|Subchapter)", header_tag.text):
                        artical_nums = re.search(r'^(Article|SUBCHAPTER(S)*|Subchapter)\s(?P<chapter_id>\w+?\d+?)',
                                                 header_tag.text.strip()).group(
                            'chapter_id').zfill(2)
                        header_tag.name = "h2"
                        header_tag.attrs = {}
                        ...
                        if re.search(r'SUBCHAPTER|Subchapter', header_tag.text.strip()):
        ...
        for p_tag in self.soup.find_all():
            ...
            if re.search(rf'^\({ol_head}\)', current_tag_text):
                ...
                if re.search(r'^a\.', current_tag_text) :
                    ...
                    if  re.search(r'^ARTICLE',p_tag.find_previous("h3").get_text().strip()) and not num_tag:
                        p_tag.wrap(innr_alpha_ol)
                        prevnum_id1 = p_tag.find_previous("h3").get("id")
                    else:
                        p_tag.wrap(innr_alpha_ol)
