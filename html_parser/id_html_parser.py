...
class idParseHtml(ParserBase):
    def __init__(self, input_file_name):
        ...
        self.tag_type_dict = {'head1': r'^Title \d', 'head3': r'^§\s?\d+-\d+\.|^§\s?\d+-\d+-\d+\.',
                              'head4': '^STATUTORY NOTES',
                             'junk': 'Title \d', 'ul':r'^Idaho Code Title \d','normalp': '^Editor\'s note',
    ...
    def remove_junk(self):
        ...
        [text_junk.decompose() for text_junk in self.soup.find_all("p", class_=self.tag_type_dict["ul"])
         if re.search(r'^Idaho Code (Title|Ch\.|§)|^Idaho Code Pt\. \d+',text_junk.text.strip())]
    ...
    def replace_tags(self):
        ...
        for header_tag in self.soup.body.find_all():
            if header_tag.get("class") == [self.tag_type_dict['head1']]:
                if re.search(r'Title\s(?P<title>\d+)',header_tag.get_text()):
                    header_tag.name = "h1"
                    header_tag["id"] = f't{self.title}'
                    self.snav_count = 1
                    header_tag.wrap(self.soup.new_tag("nav"))

                elif chap_head := re.search(r'Part\s?(?P<c_title>\d+)', header_tag.get_text()):
                    header_tag.name = "h2"
                    ...
                    header_tag["id"] = f'{prev_id}c{chap_head.group("c_title").zfill(2)}'
                    sec_count = 1
                    self.snav_count = 1


                elif chap_head := re.search(r'Chapter\s(?P<c_title>\d+[a-zA-Z]?)',header_tag.get_text()):
                    header_tag.name = "h2"
                    header_tag["id"] = f't{self.title}p{chap_head.group("c_title").zfill(2)}'
                    sec_count = 1
                ...
                if sec_head := re.search(r'^ARTICLE(\s?)(?P<sec_id>[IVX]+)', header_tag.get_text()):
                    header_tag.name = "h3"
                    ...
                    if re.search(r'^History\.', header_tag.get_text()):
                        ...
                        if subsec_head_id in sub_sec_id:
                            header_tag[
                                "id"] = f'{header_tag.find_previous("h4").get("id")}-{header_tag_text}{sub_sec_count}'
                            sub_sec_count += 1
                        else:
                            header_tag["id"] = f'{header_tag.find_previous("h4").get("id")}-{header_tag_text}'
            ...
            if header_tag.get("class") == [self.tag_type_dict['ul']]:
                if re.search(r'^\d+-\d+[A-Z]?\.\s[A-Z\[]|^\d+-\d+\s?[—,]\s\d+-\d+|'
                             r'^\d+-\d+[a-zA-Z]?(-\d+)?\.?',header_tag.get_text()) or (re.search(r'^\d+\.|Chapter \d+\.',header_tag.get_text()) and not header_tag.find_previous("h3")) :
                    header_tag.name = "li"
                ...
                elif re.search(r'^Chapter',header_tag.get_text()):
                    header_tag.find_previous("nav").append(header_tag)

                elif re.search(r'^Cited', header_tag.get_text()):
                    header_tag.name = "h4"
                    if header_tag.find_previous("h3"):
                        cite_head_id = f'{header_tag.find_previous("h3").get("id")}-{header_tag.get_text()}'
                    else:
                        cite_head_id = f'{header_tag.find_previous(["h2","h1"]).get("id")}-{header_tag.get_text()}'

                    if cite_head_id in cite_id:
                        header_tag["id"] = f'{header_tag.find_previous("h3").get("id")}-{header_tag.get_text()}.{cite_count}'
                        cite_count += 1
                    else:
                        header_tag["id"] = f'{header_tag.find_previous("h3").get("id")}-{header_tag.get_text()}'

                    sub_sec_count = 1
                    cite_id.append(cite_head_id)

            elif header_tag.get("class") == [self.tag_type_dict['head']]:
                header_tag.name = "h5"
                ...
                if re.search(r'^History\.', header_tag.get_text()):
                    ...
                    if header_tag.find_previous("h4"):
                        ...
                        subsec_head_id = f'{header_tag.find_previous(["h2","h1"]).get("id")}-{header_tag_text}'
    ...
    def create_ul_tag_and_case_note_nav(self):
        ...
        for list_item in self.soup.find_all("li"):
            if list_item.find_previous().name == "li" and not list_item.parent.name == "ul":
                ...
                if re.search(r'^(Chapter\s?)*\d+\.',list_item.get_text()):
    ...
    def create_chapter_section_nav(self):
        for list_item in self.soup.find_all("li", class_=self.tag_type_dict['ul']):
            if sec_list := re.search(r'^(?P<chap_id>\d+-\d+[a-zA-Z]?(-\d+)?)', list_item.get_text()) :
                nav_list = []
                ...
                list_item["id"] = f"{list_item.find_previous('h2').get('id')}s{sec_list.group('chap_id')}-snav{self.snav_count:02}"
                self.snav_count += 1
    ...
    def recreate_tag(self):
        ...
        for p_tag in self.soup.findAll("p",class_=self.tag_type_dict["ul"]):
            current_p_tag = p_tag.get_text()
            if re.search(r'^Cited', p_tag.get_text()):
            ...
            # a
                ...
                if re.search(r'^\(a\)|^a\.', current_tag_text) :
                    ...
                    if num_cur_tag:
                        ...
                        prevnum_id = prev_num_id
                        p_tag["id"] = f'{prevnum_id}{main_sec_alpha}'
                    else:
                        prevnum_id =p_tag.find_previous(["h4", "h3"]).get("id")
                        p_tag["id"] = f'{p_tag.find_previous(["h4", "h3"]).get("id")}{main_sec_alpha}'
                else:
                ...
                if re.search(r'^\(\w\)\s?\([ivx]+\)', current_tag_text):
                    ...
                    innr_roman_ol["id"] = f'{prev_head_id}ol{ol_count}{cur_tag.group("cid")}{cur_tag.group("pid")}'
